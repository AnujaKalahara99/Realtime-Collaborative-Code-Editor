...
const Consumer = () => {
  ...
  return (
    <div>
      ...
      <span data-testid="loading">{context.loading ? "loading" : "not-loading"}</span>
...
describe("CodespaceContext", () => {
  ...
  it("provides default values", async () => {
    ...
    expect([null, "", undefined, ""].includes(screen.getByTestId("error").textContent)).toBe(true);
  });

  it("creates a codespace", async () => {
    const codespace = { id: "1", name: "TestSpace", lastModified: "now", created_at: "now", owner: "Test User", role: "owner" };
    (global.fetch as jest.Mock) = jest.fn()
      // Initial fetchCodespaces (empty)
      .mockResolvedValueOnce({ ok: true, json: async () => ({ codespaces: [] }) })
  ...
  it("deletes a codespace", async () => {
    const codespace = { id: "2", name: "ToDelete", lastModified: "now", created_at: "now", owner: "Test User", role: "owner" };
    (global.fetch as jest.Mock) = jest.fn()
      // Initial fetchCodespaces (empty)
      .mockResolvedValueOnce({ ok: true, json: async () => ({ codespaces: [] }) })
      // createCodespace (returns new codespace)
  ...
  it("edits a codespace name", async () => {
    const codespace = { id: "3", name: "OldName", lastModified: "now", created_at: "now", owner: "Test User", role: "owner" };
    (global.fetch as jest.Mock) = jest.fn()
      // Initial fetchCodespaces (empty)
      .mockResolvedValueOnce({ ok: true, json: async () => ({ codespaces: [] }) })
      // createCodespace (returns new codespace)
  ...
  it("shares a codespace by email", async () => {
    ...
    let result;
    await act(async () => {
      result = await contextRef.shareCodespaceByEmail("id", "test@email.com", "editor");
    });
    expect(result).toBe(true);
  });
  ...
  it("handles invalid email in shareCodespaceByEmail", async () => {
    ...
    let result;
    await act(async () => {
      result = await contextRef.shareCodespaceByEmail("id", "bademail", "editor");
    });
