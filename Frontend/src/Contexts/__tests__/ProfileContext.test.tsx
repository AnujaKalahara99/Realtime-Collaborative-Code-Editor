...
jest.mock("../../database/superbase", () => {
  ...
  return {
    supabase: { auth, from },
    __setSingleImpl: (impl: jest.Mock<any, any, any>) => { singleImpl = impl; },
    __setUpdateImpl: (impl: jest.Mock<any, any, any>) => { updateImpl = impl; },
  };
});

const { supabase, __setSingleImpl, __setUpdateImpl } = require("../../database/superbase");

// Helper to mock supabase.from().select().eq().single()
function mockSupabaseSingle(returnValue: { data: { id: string; first_name: string; last_name: string; email: string; bio: string; location: string; job_title: string; phone: string; website: string; tech_stacks: string[]; avatar_url: string; } | { id: string; first_name: string; last_name: string; email: string; bio: string; location: string; job_title: string; phone: string; website: string; tech_stacks: string[]; avatar_url: string; } | null; error: { message: string; } | null; }) {
  const single = jest.fn().mockResolvedValue(returnValue);
  __setSingleImpl(single);
  return single;
}
// Helper to mock supabase.from().update().eq()
function mockSupabaseUpdateWithEq(returnValue: { error: { message: string; } | null; }) {
...
describe("ProfileContext", () => {
  const TestComponent = () => {
    const { profileData, updateProfile, loading, error, refreshProfile } = useProfile();
    return (
      <div>
        <span data-testid="firstName">{profileData.firstName}</span>
        <span data-testid="loading">{loading ? "loading" : "not-loading"}</span>
        <span data-testid="error">{error}</span>
        <button onClick={() => updateProfile({ firstName: "Jane" })}>Update</button>
  ...
  it("loads profile data on mount", async () => {
    supabase.auth.getUser.mockResolvedValue({ data: { user: { id: "1", email: "test@example.com" } } });
    mockSupabaseSingle({
      data: {
        id: "1",
        first_name: "John",
    ...
    await waitFor(() => expect(getByTestId("firstName").textContent).toBe("John"));
  ...
  it("handles error loading profile", async () => {
    ...
    await waitFor(() => expect(getByTestId("error").textContent).toBe("Failed to load profile data"));
  });
  ...
  it("handles update error", async () => {
    ...
    await waitFor(() => expect(getByTestId("error").textContent).toBe("Failed to update profile"));
  });
