...
import { useNavigate } from "react-router";
import { FileText, MoreVertical, Trash2, Share2, Edit, Link } from "lucide-react";
import { useTheme } from "../ThemeProvider";
import { type Codespace, type ViewMode } from "./codespace.types";
import { useState } from "react";
  ...
  return (
    <>
      <div
        onClick={handleClick}
        onMouseLeave={handleMouseLeave}
        className={`${theme.surfaceSecondary} rounded-lg ${theme.border} border ${theme.hover} cursor-pointer transition-all duration-200 hover:shadow-md relative ${
          viewMode === "grid" ? "p-6 min-h-[200px] flex flex-col" : "p-4 flex items-center justify-between"
        }`}
      >
        <div
          className={`${viewMode === "grid" ? "flex-1" : "flex items-center space-x-4"}`}
        >
          <div
            className={`${theme.surfaceSecondary} rounded-lg p-3 w-fit ${viewMode === "list" ? "!p-2" : ""}`}
          >
            <FileText size={viewMode === "grid" ? 32 : 20} className="text-blue-500" />
          </div>

          <div className={`${viewMode === "grid" ? "mt-4" : ""}`}>
            <h3
              className={`font-medium ${theme.text} ${viewMode === "grid" ? "text-lg mb-2" : "text-base"}`}
            >
              {displayName}
            </h3>
            <div
              className={`text-sm ${theme.textMuted} ${viewMode === "grid" ? "space-y-1" : "flex items-center space-x-4"}`}
            >
              <p>Modified {codespace.lastModified}</p>
              <p>{codespace.role}</p>
            </div>
        ...
        {(onDelete || onEdit || onShare) && (
          <button
            onClick={toggleMenu}
            className={`${theme.hover} p-2 rounded-lg transition-colors ${viewMode === "grid" ? "self-end mt-4" : ""} hover:bg-gray-100 dark:hover:bg-gray-700`}
          >
            <MoreVertical size={16} className={theme.textMuted} />
          </button>
        )}
        ...
        {showMenu && (
          <div
            className={`${theme.surface} absolute right-4 ${viewMode === "grid" ? "top-[calc(100%+8px)]" : "top-[calc(100%+4px)]"} z-50 border ${theme.border} rounded-md shadow-lg p-2 w-48`}
            onClick={(e) => e.stopPropagation()}
          >
            {onEdit && (
              <button
