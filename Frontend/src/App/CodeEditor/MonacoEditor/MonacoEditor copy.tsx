...
export default function MonacoEditor({
  ...
  console.log("Rendering MonacoEditor with selectedFile:", selectedFile);
  ...
  useEffect(() => {
    console.log("Initializing VFS Monaco Integration...", vfsBridge);
      ...
      {selectedFile && selectedFile.type === "file" && (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            {getUsersInFile(selectedFile.id).length > 0 && (
              <div className="flex items-center gap-1">
                <div className="flex -space-x-1">
                  {getUsersInFile(selectedFile.id)
                    .slice(0, 3)
                    .map((user, index) => (
                      <div
                        key={index}
                  ...
                  {getUsersInFile(selectedFile.id).length > 3 && (
                    <div
                ...
                <span className={`text-xs ${theme.textMuted}`}>
                  {getUsersInFile(selectedFile.id).length} user
                  {getUsersInFile(selectedFile.id).length !== 1 ? "s" : ""}
                </span>
              </div>
            )}
          </div>
