...
describe("CodespaceCard", () => {
  ...
  test("shows and hides menu, triggers Edit modal", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText(/Edit Codespace Name/i)).not.toBeInTheDocument()
    );
  });
  ...
  test("shows and hides Share modal", async () => {
    ...
    fireEvent.click(screen.getByText(/Cancel/i));
    await waitFor(() =>
      expect(screen.queryByText(/Share Codespace/i)).not.toBeInTheDocument()
    );
  });
  ...
  test("delete codespace", async () => {
    ...
    await waitFor(() => expect(deleteCodespace).toHaveBeenCalledWith("1"));
  });
